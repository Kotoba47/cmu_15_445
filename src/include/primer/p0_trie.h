...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    ...
    return &((*this).children_[key_char] = std::move(child));
  }
 ...
 public:
  ...
  bool Remove(const std::string &key) {
    ...
    for (size_t i = nodes.size() - 1; i >= 0; --i) {
      ...
      if ((*current)->HasChildren() || (*current)->IsEndNode()) {
        break;
      }

      std::unique_ptr<TrieNode> *parent = nodes.back();
  ...
  T GetValue(const std::string &key, bool *success) {
    *success = false;
    ...
    auto node = dynamic_cast<TrieNodeWithValue<T> *>(current->get());
    if (node == nullptr) {
      latch_.RUnlock();
      return {};
    }
    *success = true;
    auto result = node->GetValue();
    latch_.RUnlock();
    return result;
  }
};
